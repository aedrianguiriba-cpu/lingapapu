...
<div class="app-content">
  ...
  <div class="tab-panel active" id="tab-id">
    ...
    <div style="margin-top:16px;padding:16px;background:#f0fdf4;border:1.5px solid #86efac;border-radius:14px" id="bioRegistrationSection">
...
async function registerBiometric() {
  if (!window.PublicKeyCredential) {
    showToast('⚠️ Biometric login is not supported on this device. You can only log in with your password.', 'error');
    return;
  }
  ...
  let hasSensor = false;
  try {
    hasSensor = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
  } catch (e) {
    console.warn('Could not check sensor availability:', e);
  }
  
  if (!hasSensor) {
    showToast('⚠️ No biometric sensor (fingerprint/face recognition) detected on this device. Password login only.', 'error');
    return;
  }
  ...
  try {
    ...
    if (e.name === 'NotAllowedError') {
      ...
      showToast('Biometric authentication is not supported on this device. Password login only.', 'error');
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  const status = document.getElementById('bioRegStatus');
  const bioBtn = document.getElementById('registerBioBtn');
  if (!status) return;
  
  // Check if biometric is supported on this device
  const isBiometricSupported = !!window.PublicKeyCredential;
  if (!isBiometricSupported) {
    status.textContent = '📱 Biometric login not supported on this device';
    status.style.color = '#9ca3af';
    if (bioBtn) {
      bioBtn.disabled = true;
      bioBtn.style.opacity = '0.5';
      bioBtn.style.cursor = 'not-allowed';
      bioBtn.title = 'Your device does not support biometric authentication. You can still log in with your password.';
    }
    return;
  }
...
function renderIDCard() {
  ...
  const reg = currentUser.registrationDate || currentUser.dateRegistered || currentUser.registration_date || currentUser.createdAt || currentUser.created_at;
...
function initBiometricSupport() {
  ...
  const bioMsg = document.getElementById('bioUnsupportedMsg');
  const divider = document.querySelector('.face-login-divider');
  
  if (!bioBtn) return;
  
  const isBioSupported = !!window.PublicKeyCredential;
  
  if (!isBioSupported) {
    // Hide biometric option entirely on unsupported devices
    bioBtn.style.display = 'none';
    if (divider) divider.style.display = 'none';
    if (bioMsg) bioMsg.style.display = 'block';
  }
}

// Call on DOM ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initBiometricSupport);
} else {
...
async function openFaceLogin() {
  ...
  let hasBioSensor = false;
  try {
    hasBioSensor = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
  } catch (e) {
    console.warn('Could not check biometric availability:', e);
  }
  
  if (!hasBioSensor) {
    const m = document.getElementById('loginError');
    m.textContent = '📱 No biometric sensor detected on this device. Please use your username and password above.';
    m.classList.add('show');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      _showBioModal('error', 'Login failed', 'Please try password login instead.');
    }
  } catch (e) {
    if (e.name === 'NotAllowedError') {
      _showBioModal('error', 'Cancelled', 'Biometric authentication cancelled. Use your password to login.');
    } else if (e.name === 'NotSupportedError' || e.name === 'NotAvailableError') {
